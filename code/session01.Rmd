...
authors: "Valentin Geier & Zoé Wolter"
date: "July 12, 2022"
...
- `readxl`: The `reaxl`-package also belongs to the tidyverse and enables us to conveniently import Excel-files (.xls and .xlsx).
...
```{r}
aiddata_preprocessed <- read_csv(here::here("data/AidData_CDF_2021_preprocessed.csv"))
```

# 1.3 First glimpse at the data

Executing the following three commands provides us with a first glimpse at the data and its structure.
...
Before diving into the visualizations, we will do some data wrangling to create two data sets that are then ready for some nice visualizations.
...
# With `drop_na()` we drop all cases for which we do not know the actual amount of financing
  drop_na(amount) %>%
# Using functions provided by the `lubridate` -package, we transform the values in the year-columns into a date-time format
...
# We also divide the values in the column indicating the amount of money spent on individual projects by 1 billion to make handling and visualizing the data easier.
         amount_bn = amount/1000000000,
# We set the targeted sector of the financing as a factor, and rearrange its order. This is relevant at a later stage.
         sector = factor(sector),
         sector = fct_relevel(sector, c("COMMUNICATIONS","EDUCATION", "ENERGY", "HEALTH", "INDUSTRY, MINING, CONSTRUCTION", "TRANSPORT AND STORAGE", "WATER SUPPLY AND SANITATION",  "OTHER")))
...
# Then, we calculate the total amount of development financing the Chinese government committed to spending per year
            amount_bn_total = sum(amount_bn),
# And lastly, we calculate the mean amount of development finance committed to per year
            amount_bn_mean = mean(amount_bn))
```
...
Set up a ggplot using `ggplot()` by piping (%>%) the 'aiddata_yearly' dataset into the function's `data` argument. 
...
This code chunk creates a blank canvas in the 'Plots' panel of RStudio, since we have yet to indicate the arguments to be used for the plotting. 
We do this by specifying the `mapping` argument using the `aes()` helper function.

# 2.2 Specifying the parameters

We now want the x-values of the plot to indicate the year in which the Chinese government committed to delivering the aid financing (`commitment_year`), and the y-values to represent the number of projects the Chinese government committed to (`n_projects`).
...
Both the bar plot as well as the line plots show an upward trend in the number of financial aid commitments of the Chinese government between 2000 and 2017. Often times, line plots are a handy tool to display such trends.

# 2.3.3 Line plots with geom_line und geom_smooth

With `geom_line` we can visualize a simple line plot, since the function connects observations in order of the variable on the x-axis. Beyond, `geom_smooth` displays smoothed conditional means that can help us to identify trends in data. By default, it also shows confidence intervals - these can be omitted using the `se = FALSE`- command.
...
Now, start creating some plots yourself. Again, we want to display the years on the x-axis. On the y-axis, we now want to visualize the amount of development finances committed to by the Chinese government (`amount_bn_total`).  
...
If you have finished early, feel free to play around with some other geometric shapes. To have a glimpse at all the possibilities ggplot2 offers, take a look at the 'Cheatsheet' provided in the folder.

```{r}
# Maybe step plot could be a handy visualization for the association between year and project number?
...
We now turn to a more fine-grained version of the data set provided by AidData which, besides the sole number of projects or the amount of aid, contains quite some interesting additional information such as the recipient, the recipient region, the intent and the targeted sector.
...
Our dataset i.a. contains information on the purpose of financial transfers (e.g. development, commercial, etc.). We can introduce this information into our plot by using the `facet_wrap`-function. 
For each manifestation of a discrete variable that we specify by using `~ ` inside `facet_wrap()`, a separate panel is created in the plot. By default, `facet_wrap()` uses the same scale for each panel. Often times, this is appropriate and makes comparisons easier. In our case, however, the amounts spent for `Representational` purposes are so small, that the respective panel needs a different scale on the y-axis. By setting `scale = "free"`, `ggplot2` uses the most appropriate scale for each panel.
...
Despite using `theme_minimal()`, there remain a number of tweaks that could make our plot look even nicer. We can thus specify `theme()` AFTER `theme_minimal()` to make some manual adjustments. These include changes to the look of both title (`plot.title`) and subtitle(`plot-subtitle`), the removal of the legend's title (`legend.title`) and it's relocation (`legend.position`) to the bottom of the plot. To simply omit (NON-DATA) components of the plot, we use `element_blank`, while making changes to text components requires the command `element_text`.
...
Below, switching from `theme_minimal` to `theme_wsj` has totally messed up the plot. Can you find a way to modify the code in such a way, that it looks decent again?
...
aiddata_red %>% 
 ggplot(aes(x = commitment_year,
  ...
  labs(x = "Year",
       y = "Aid (in billion 2017 US$)",
       title = "Chinese development financing",
       subtitle = "By sector between 2000 and 2017",
       caption = "Source: AidData (2021)") +
  theme_wsj() +
...
In the following, we have used `RColorBrewer` and a color palette that, thanks to its contrasts, is especially suitable for qualitative scales ("Accent").
...
In the plot below, we aim to visualize over-time variance in financing activities that have a clear-cut development `intent` between 2010 and 2017 by `region`.
Currently, however, both the x- and the y-axis are not appropriately scaled yet. Furthermore, a more concise color pattern suited for qualitative scales such as different regions would improve our plot as well. Try to improve the visualization based on the following code chunk by adding properly specified `scale_x_date()`-, `scale_y_continuous()` and ` scale_color_brewer()`-commands.

```{r}
# We need some minor prior data wrangling to have the data ready for our desired plot
aiddata_red %>%
  filter(intent == "Development", 
         region != "Multi-Region", 
         commitment_year >= "2010-01-01") %>%
  group_by(commitment_year, region) %>%
  summarise(amount_bn_dev = sum(amount_bn)) %>%
# Here begins the actual plotting
  ggplot(aes(x = commitment_year,
             y = amount_bn_dev,
             color = region)) + 
  geom_point() +
  ...
  labs(x = "Year",
       y = "Aid (in billion 2017 US$)",
       title = "Chinese development financing",
       subtitle = "Development aid by region between 2010 and 2017", 
       caption = "Source: AidData (2021)") +
# Our x-axis is based on a date-time scale. Thus we need to use scale_x_date(). Specify it, so that date_breaks represent one year and the date_labels indicate only the year
  
   
# Our y-axis is based on a continuous scale, namely the amount of financial aid in billion. Define it, so that we have 6 breaks on a scale from 0 to 10
   
  
# Now, we want to manually adapt the color scale. We want to do this by using the `RColorBrewer`-package and it's palette called "Set1". CAREFUL, we have used the "color ="-argument above, wherefore "scale_fill_brewer()" doesn't work anymore
...
One last crucial step is to save and export your plot for later use in presentations, essays, or term papers. The `ggsave()`-function provided by the `ggplot2`-package comes in really handy in that regard. I.a. it allows us to store our plots in different file formats, ranging from "pdf" and "jpeg" to "svg". Storing our plot as a "svg" (~ scalable vector graphic) should generally be the preferred option as these files can be scaled in size without loss of quality.

```{r}
# First of all, we have to save our plot as an object in R.
aiddata_plot <-
  aiddata_red %>% 
  ggplot(aes(x = commitment_year,
             y = amount_bn,
...
# We now have a list object called "aiddata_plot" saved in our environment that we can easily export using `ggsave()`
ggsave(filename = "aiddata_plot.svg", # First, we specify the filename (notice the ".svg" in the end).
       plot = aiddata_plot, # Here we choose which plot we want to export.
       device = "svg") # And lastly we choose the the device/filetype.
```

The styling we we have resorted to for our plotting today is just one out of infinite options for how to make your data visually appealing. In the following you might want to try to develop your very own style. 
Feel free to again visualize over-time variance in financing activities that have a clear-cut development `intent` between 2010 and 2017 by `region`. 
